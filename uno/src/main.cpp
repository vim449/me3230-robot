...
void menu() {
    ...
    Serial.println(
        "Options are Drive: 'a', Press Button 'b', discard 'c', "
        "dispense 'd', move rack 'e', line follow 'l', coast 'C', "
        "sense color 's', path follow 'p', start competition code 'S'");
}
...
void menu(State submenu) {
    ...
    case pressButton:
        Serial.println(
            "Please enter how many times the button should be pressed");
        break;
    case dispensing:
        Serial.println(
...
void pathFunction() {
    ...
    for (int i = 0; i < segments; i++) {
        ...
        case 's':
            Serial.print(
                "Please enter the distance (m) the robot should drive");
            break;
        case 'p':
        ...
        case 'h':
            Serial.print(
                "Please enter the radius (m) of swing turn the robot should "
        ...
        if (Serial.available() > 0) {
            ...
            if (arg != '\n') {
                if (!needParam) {
                    ...
                    switch (arg) {
                    case 'a':
                        menu(drivingTimeBased);
                        state = drivingTimeBased;
                        needParam = true;
                        break;
                    case 'b':
                        menu(pressButton);
